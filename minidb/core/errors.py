...
DUPLICATE_UPDATE_COLUMN = "DUPLICATE_UPDATE_COLUMN"
INVALID_DATE = "INVALID_DATE"
RESERVED_NAME = "RESERVED_NAME"
TYPE_MISMATCH = "TYPE_MISMATCH"
UNSUPPORTED_COMPARISON = "UNSUPPORTED_COMPARISON"
CONDITION_NOT_BOOL = "CONDITION_NOT_BOOL"
# 值与类型细化错误：写前由语义层报出，RowCodec 复核已归一化值时按 STORAGE 报出。
VALUE_TOO_LONG = "VALUE_TOO_LONG"
NUMERIC_OUT_OF_RANGE = "NUMERIC_OUT_OF_RANGE"
NUMERIC_SCALE_MISMATCH = "NUMERIC_SCALE_MISMATCH"
# 定义期的约束与类型参数错误：由 Schema/Semantic 在读改写前报出。
...
# ---- 关系约束（STORAGE）----
# ConstraintValidator 在写前探测自动索引时使用，必须与 NOT_NULL_VIOLATION
# 一样在写事务内按 STORAGE 报出。
...
INDEX_KEY_TOO_LARGE = "INDEX_KEY_TOO_LARGE"
...
ALL_ERROR_CODES: frozenset[str] = frozenset({
    ...
    INVALID_TRANSACTION_STATE, PAGE_CORRUPTED, DB_FORMAT_MISMATCH, DB_FILE_TRUNCATED,
    CATALOG_CORRUPTED, ID_EXHAUSTED, INDEX_CORRUPTED, INDEX_KEY_TOO_LARGE,
    # I/O
...
ERROR_STAGE_BY_CODE: dict[str, ErrorStage] = {
    ...
    DUPLICATE_CONSTRAINT: ErrorStage.SEMANTIC,
    INSERT_COLUMN_SET_MISMATCH: ErrorStage.SEMANTIC,
    VALUE_COUNT_MISMATCH: ErrorStage.SEMANTIC,
    MISSING_REQUIRED_COLUMN: ErrorStage.SEMANTIC,
    DUPLICATE_UPDATE_COLUMN: ErrorStage.SEMANTIC,
    INVALID_DATE: ErrorStage.SEMANTIC,
    ...
    # 写前报 SEMANTIC，RowCodec 复核时允许 STORAGE（见 _RECHECK_STAGES_BY_CODE）。
    VALUE_TOO_LONG: ErrorStage.STORAGE,
    NUMERIC_OUT_OF_RANGE: ErrorStage.STORAGE,
    NUMERIC_SCALE_MISMATCH: ErrorStage.STORAGE,
    CONFLICTING_CONSTRAINT: ErrorStage.SEMANTIC,
    INVALID_TYPE_PARAMETER: ErrorStage.SEMANTIC,
    DUPLICATE_CONSTRAINT: ErrorStage.SEMANTIC,
    DUPLICATE_UPDATE_COLUMN: ErrorStage.SEMANTIC,
    INDEX_EXISTS: ErrorStage.SEMANTIC,
    INDEX_NOT_FOUND: ErrorStage.SEMANTIC,
    # 计划
    ...
    NOT_NULL_VIOLATION: ErrorStage.STORAGE,
    PRIMARY_KEY_VIOLATION: ErrorStage.STORAGE,
    UNIQUE_VIOLATION: ErrorStage.STORAGE,
    ...
    DATABASE_BUSY: ErrorStage.STORAGE,
    TRANSACTION_REQUIRED: ErrorStage.STORAGE,
    INVALID_TRANSACTION_STATE: ErrorStage.STORAGE,
    PAGE_CORRUPTED: ErrorStage.STORAGE,
    INDEX_CORRUPTED: ErrorStage.STORAGE,
    INDEX_KEY_TOO_LARGE: ErrorStage.STORAGE,
...
_RECHECK_STAGES_BY_CODE = {
    ...
    # 计划 10.2：值超限写前报 SEMANTIC，写事务内或 Codec 复核时按 STORAGE 报出。
    VALUE_TOO_LONG: ErrorStage.SEMANTIC,
    NUMERIC_OUT_OF_RANGE: ErrorStage.SEMANTIC,
    NUMERIC_SCALE_MISMATCH: ErrorStage.SEMANTIC,
}
