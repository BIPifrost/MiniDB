...
* ``core/disk_types.py`` from Liao Jie will be the single source of PAGE_SIZE,
  INVALID_PAGE_ID and disk format version; do not duplicate those here.
...
class DataPageHeader:
    ...
    def __post_init__(self) -> None:
        ...
        _require_int_range("next_page_id", self.next_page_id, 1, 0xFFFFFFFF)
        _require_int_range("slot_count", self.slot_count, 0, 0xFFFF)
        _require_int_range("free_start", self.free_start, DATA_PAGE_HEADER_SIZE, 0xFFFF)
        _require_int_range("free_end", self.free_end, DATA_PAGE_HEADER_SIZE, 0xFFFF)
