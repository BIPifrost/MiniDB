...
def _catalog_context(operation: str, **details):
    ...
    try:
        ...
        if isinstance(context, dict):
            ...
            context.setdefault("table_name", SYSTEM_CATALOG_TABLE.ref.name)
        raise
...
def _record_cleanup_error(primary: BaseException, cleanup: Exception) -> None:
    ...
    if isinstance(context, dict) and all(hasattr(cleanup, key) for key in ("stage", "code", "message", "context")):
