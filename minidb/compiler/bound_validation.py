...
def validate_expression(expr, table, check: Check) -> None:
    """使用显式栈遍历，所有分支都检查，不能按 AND/OR 的值短路。"""
    # 每项是 (节点, 是否离开节点)。离开节点时，其子节点已全部通过检查。
    pending = [(expr, False)]
    active: set[int] = set()
    while pending:
...
def validate_predicate(predicate, table, check: Check) -> None:
    """None 表示没有过滤条件；非空条件必须结构合法且最终类型为 BOOL。"""
    if predicate is not None:
        validate_expression(predicate, table, check)
...
def validate_bound(bound, *, plan: bool = False) -> None:
    ...
    elif isinstance(bound, BoundSelect):
        check.projection(bound.table, bound.projection, bound.output_columns)
        validate_predicate(bound.predicate, bound.table, check)
